...
from horizon.horizon_pricer.models.pricer.module_interpolation import interpolation_tenors_bam, get_volume_courbe_strates
...
def get_tenors_main(gen_date_ini, gen_date_fin) -> pd.DataFrame:
    dcourbe = query_courbe_range(gen_date_ini, gen_date_fin, dbs)
    dcourbe = dcourbe.sort_values('date_marche').drop_duplicates()
    dico_res = {}
    for el_date in dcourbe['date_marche'].unique():
        try:
            dg = query_tenors(el_date, dbs)
            dico = dict(zip(dg['tenors_y'], dg['taux']))
            dico_res[el_date] = dico
        except Exception as e:
            print(f"Error processing date {el_date}: {e}")
    ...
    dres = pd.DataFrame.from_dict(dico_res, orient='index').reset_index()
